...
const Payment = () => {
  ...
  const onAddPayment = (row) => {
    // Open modal with row data pre-filled
  ...
  const columns = useMemo(
    () => [
      ...
      // {
      //   name: <span className="font-semibold">Payment Type</span>,
      ...
      //   selector: (row) => row.PaymentDtTm || "-",
      //   sortable: true,
      //   cell: (row) => {
      //     if (!row.PaymentDtTm) return <div className="text-slate-500">-</div>;
      //     try {
      //       const dateObj = new Date(row.PaymentDtTm);
      //       return (
      //         <div className="font-medium text-slate-800">
      ...
      //       );
      //     } catch {
      //       return <div className="text-slate-500">{row.PaymentDtTm}</div>;
      //     }
      //   },
      // },
      {
                        ...
                        <tbody className="bg-white divide-y divide-slate-200">
                          {viewData.map((item, index) => (
                            <tr key={item.PaymentID || index} className="hover:bg-blue-50 transition-colors">
                              <td className="px-4 py-3 text-sm font-medium text-slate-800 border-r border-slate-200">
                                {item.PaymentID || "-"}
                              </td>
